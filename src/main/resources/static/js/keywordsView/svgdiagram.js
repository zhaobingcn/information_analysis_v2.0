...
function changeDivWidth(){
    ...
    var svg = d3.select("svg"),
        margin = 100,
        diameter = document.getElementsByTagName("svg").width,  //直径等于<svg>的属性width值
        //<svg>中添加<g>标签
        g = svg.append("g").attr("transform", "translate(" + diameter / 2 + "," + (diameter / 4) + ")");
    ...
    d3.json("/allKeywords/showSvg", function(error, root) {
        ...
        zoomTo([root.x, root.y, root.r * 4.5 + margin ]);  //页面加载完成初始位置
        ...
        function zoom(d) {
            ...
            var transition = d3.transition()  //创建一个过渡对象
                ...
                .tween("zoom", function(d) {
                    ...
                    var i = d3.interpolateZoom(view, [focus.x, focus.y, focus.r * 4.5 + margin]);
