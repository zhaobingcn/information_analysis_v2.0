...
public class ConvertToNode {
    ...
    public Map<String, Object> getPaper(JSONObject object){
        ...
        if(object.get("quote") != null){
            paper.put("quote", Integer.parseInt(object.get("quote").toString()));
        }
        ...
        if(object.get("date").getClass().equals(JSONObject.class)){
            ...
            //period有字母的问题
            Integer period = 1;
            if(date.getString("period").charAt(0) >= 'a'
                    &&
                    date.getString("period").charAt(0) <= 'z'
                    ||
                    date.getString("period").charAt(0) >= 'A'
                    &&
                    date.getString("period").charAt(0) <= 'Z'
                    ){
                period = Integer.parseInt(date.getString("period").substring(1));
            }else if(!date.getString("period").contains("_")){
                period = Integer.parseInt(date.getString("period"));
            }
